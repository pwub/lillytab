...
public class ABoxNodeTest
{
	...
	/**
	 * Test of getSuccessors method, of class ABoxNode.
	 */
	@Test
		...
		throws EReasonerException, ENodeMergeException
	{
		...
		_aboxNode.getSuccessors().put("r", node2.getNodeID());
		assertTrue(_aboxNode.getSuccessors().get("r").contains(node2.getNodeID()));
		IABoxNode<String, String, String> klone = _aboxNode.clone(box2);
		assertTrue(klone.getSuccessors().get("r").contains(node2.getNodeID()));
	}

	/**
	 * Test of getPredecessors method, of class ABoxNode.
	 */
	@Test
		...
		throws EReasonerException, ENodeMergeException
	{
		...
		assertTrue(node2.getPredecessors().get("r").contains(_aboxNode.getNodeID()));
	}
		...
		throws EReasonerException, ENodeMergeException
	{
		...
		node1.getSuccessors().put("r", node3.getNodeID());

		/* not necessarily */
		assertFalse(node1.deepHashCode() == node2.deepHashCode());

		node2.getSuccessors().put("r", node3.getNodeID());
		assertTrue(node1.deepHashCode() == node2.deepHashCode());
	}
		...
		throws EReasonerException, ParseException, ENodeMergeException
	{
		...
		for (int i = 0; i < n; ++i) {
			final IDLRestriction<String, String, String> res = _parser.parse(String.format("(implies C%s C%s)", i, i + 1));
	...
	@Test
	public void nnfUnfoldTest() throws ParseException, ENodeMergeException
	{
		final IABoxNode<String, String, String> node1 = _abox.createNode(false);
