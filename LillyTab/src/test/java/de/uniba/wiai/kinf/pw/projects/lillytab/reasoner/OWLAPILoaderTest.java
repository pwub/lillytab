...
public class OWLAPILoaderTest {
	...
	private final IDLTermFactory<OWLObject, OWLClass, OWLProperty<?, ?>> _termFactory = new OWLAPIDLTermFactory(
		_dataFactory);
	private final IABoxFactory<OWLObject, OWLClass, OWLProperty<?, ?>> _aboxFactory = new ABoxFactory<>(_termFactory);
...
//		assertEquals(XSDVocabulary.BOOLEAN.getIRI(), literal.getDatatype().getIRI());
		...
		final Collection<? extends IReasonerResult<OWLObject, OWLClass, OWLProperty<?, ?>>> results = _reasoner.checkConsistency(
			_abox);
