...
public class ABoxNodeTest {
	...
	public void testGetConceptTerms()
	{
		...
		assertTrue(_aboxNode.getTerms().contains(_termFactory.getDLIndividualReference("Node")));
	...
	public void testGetSuccessors() throws EReasonerException, ENodeMergeException
	{
		...
		IABoxNode<String, String, String, String> klone = _aboxNode.clone(box2);
		...
		throws EReasonerException, ENodeMergeException
	{
		...
		IABoxNode<String, String, String, String> node3 = _abox.createNode(false);
		...
		throws EReasonerException, ParseException, ENodeMergeException
	{
		...
		for (int i = 0; i < n; ++i) {
			final IDLClassExpression<String, String, String, String> res = _parser.parse(
				String.format("(implies C%s C%s)", i, i + 1));
			_abox.getTBox().add(res);
		}
		final IABoxNode<String, String, String, String> node1 = _abox.createNode(false);

		node1.addTerm(_termFactory.getDLClassReference("C0"));
	...
	{
		final IABoxNode<String, String, String, String> node1 = _abox.createNode(false);
		node1.addTerm(_parser.parse("(implies A B)"));
