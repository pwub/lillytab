...
public class SWRLTermFactory<Name extends Comparable<? super Name>, Klass extends Comparable<? super Klass>, Role extends Comparable<? super Role>>
	...
	{
		ISWRLNominalReference<Name, Klass, Role> nominalRef = new SWRLNominalReference<>(nomimal);
	...
	{
		final ISWRLClassAtom<Name, Klass, Role> classAtom = new SWRLClassAtom<>(klass, individual);
	...
	@Override
	public ISWRLIntersection<Name, Klass, Role> getSWRLIntersection(
		final Collection<ISWRLAtomicTerm<Name, Klass, Role>> atoms)
	{
		final ISWRLIntersection<Name, Klass, Role> intersection = new SWRLIntersection<>(atoms);
	...
	{
		final ISWRLIntersection<Name, Klass, Role> intersection = new SWRLIntersection<>(atom1, atom2);
	...
	{
		final ISWRLRule<Name, Klass, Role> rule = new SWRLRule<>(head, body);
	...
	{
		final ISWRLVariable<Name, Klass, Role> variable = new SWRLVariable<>(name);
	...
	{
		final ISWRLRoleAtom<Name, Klass, Role> roleAtom = new SWRLRoleAtom<>(role, source, target);
