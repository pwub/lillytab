 ...
 *  <p /> Nodes inside an {@link IABox} need to have a natural order. Note, that the order of nodes in an ABox is
...
public interface IABox<I extends Comparable<? super I>, L extends Comparable<? super L>, K extends Comparable<? super K>, R extends Comparable<? super R>> 
	...
	/**
	 * Create a new, anonymous node and add it to the current ABox. 
	 * For datatype nodes, the returned node is empty. Individual nodes ((@literal isDatatypeNode == false})
	 * are created with global descriptions unfolded into the newly created node.
	 * <p />
	 * The {@see NodeID} of a newly node is garantueed to be unique for the current ABox and will never be re-used. 
	 * However the returned node may not actually have that node id any more because of merging. In the case of
	 * merging, the newly created node will always be merged into an already existing node. In this case,
	 * the existing (the target node of the merge) node will be returned.
	 * 
	...
	/**
	 * Create a new, anonymous node literal node.
	...
	/**
	 * Create a new, anonymous individual (non-datatype node and add it to the current ABox. 
	 * Individual nodes ((@literal isDatatypeNode == false})
	 * are created with global descriptions unfolded into the newly created node.
	 * <p />
	 * The {@see NodeID} of a newly node is garantueed to be unique for the current ABox and will never be re-used. 
	 * However the returned node may not actually have that node id any more because of merging. In the case of
	 ...
	 * individual did not exist, it will be created and added to the current ABox. The returned node
	 * is never a datatype node. Use {@link #getOrAddDatatypeNode(java.lang.Comparable) } if you
	 * need a datatype node.
	 * <p />
	...
	/**
	 *  
	 * Retrieve or create a new named {@link IABoxNode} referencing the specified {@literal literal}. If the
	 * individual did not exist, it will be created and added to the current ABox. The returned node
	 * is always a datatype node. Use {@link #getOrAddIndividualNode(java.lang.Comparable) } if you
	 * need a non-datatype node.
	 * <p />
	 * Individual nodes are created with global descriptions unfolded into the newly created node.
	 * <p />
	 * The {@see NodeID} of a newly node is garantueed to be unique for the current ABox and will never be re-used. 
	...
	/**
	 *  The node map maps node names and node IDs to actual nodes. <p /> As the current ABox may be cloned,
	 * references to ABox nodes should never be to the real class but indirectly via node ID or node name. 
	 *
	...
	/**
	 *  Join the two nodes {@literal node1} and {@literal node2} into a single node. The result of the join is a
	 * single node containing all concepts (and thus names) of both nodes. <p /> The returned node is the target node
	 * of the join, which is <em>always</em> the smaller (as defined by the natural order) of both nodes. <p /> After
	 * the join operation, only the target node of the returned {@link NodeMergeInfo} is a valid node inside the current
	 * ABox, all other nodes have been removed and should not be accessed any more. <p /> The smaller node is
	 ...
	 * nodes, but nether jump over nodes. 
	 *
	 * @param node1 First node to merge.
	 * @param node2 Second node to merge.
	...
	/**
	 *  Retrieve the list of {@link ICollectionListener}s of the internal node set. <p /> The
	 ...
	 * {@link INodeMergeListener} is informed, when two nodes are merged into one. <p /> Note, that callers should
	 * take care when modifying the listener list, because the list also may also contain internal listeners. In
	 * particular, it is not safe to assume that the list is initially empty. <p /> Listeners are not cloned together
	 * with ABoxes. 
	 *
	...
	/**
	 *  Return the list of {@link ITermSetListener}s attached to this {@link IABox}. <p /> The
	 * {@link ITermSetListener} are informed, when the term set of a node is modified. <p /> Listeners are not cloned
	 * together with ABoxes. 
	 *
	...
	/**
	 *  Compare two {@link IABox}es through deep inspection <p /> Contrary to {@link #equals(java.lang.Object) },
