 ...
 * </p><p>
 *   Note that roles must be made known to the
 *   RBox prior to use. This is done by assigned a role one of either
 *   {@link RoleProperty#DATA_PROPERTY} or
 *   {@link RoleProperty#OBJECT_PROPERTY}.
 * </p>
 *
	...
	extends IRBox<Name, Klass, Role>
{
	 ...
	 * </p><p>
	 * To change the role type of an existing role, you
	 * must remove it, first.
	 * </p>
	 * @returns {@literal true} if the role was successfully added
	 * or {@literal false} if the role did already exist with the
	 * specified {@literal roleType}.
	 * @throws EInconsistentRBoxException The add operation
	 ...
	 * 
	 * @returns {@literal true} if the role was successfully removed
	 * or {@literal false} if the role did not exist.
	 **/
	boolean removeRole(final Role role);

	/**
	 * Change the type of {@literal role} to {@literal roleType},
	 * if possible. Inconsistencies are not resolved automatically,
	 ...
	 * </p><p>
	 * Note that removing the assertion need not remove the equivalence
	 * relationship, as this may still be derived by other means.
	 * </p>
	 * 
	...
	/**
	 * <p>
	 * Remove the equivalence assertion between the two roles {@literal first}
	 * and {@literal second}. 
	 * <p></p>
	 * Note that removing the assertion need not remove the equivalence
	 * relationship, as this may still be derived by other means.
	 ...
	 * @param second The second role
	 * 
	 * @return {@literal true} if the assertion was successfully removed and {@literal false}
	 *	if the assertion did not exist.
	 * 
	 **/
	boolean removeInverseRole(final Role first, final Role second);
	
	/**
	 * Add an inverse role assertion between {@literal first} and
	 * {@literal second}.
	 * 
	 * @param first The first role
	 * @param second The second role
	 ...
	 * </p><p>
	 *   Note that removing the assertion need not remove the subrole relationship,
	 *   as this may still be derived by other means.
	 * </p>
	 ...
	 * </p><p>
	 * Clearing an asserted role property does not affect the derived
