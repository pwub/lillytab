	...
	implements IDLSomeRestriction<Name, Klass, Role>, IUnaryOperator<IDLRestriction<Name, Klass, Role>>
{
	...
	public int compareTo(final IDLTerm o)
	{
		...
		if (compare == 0) {
			...
			/**
			 * We could include <Role> in the above cast instead
			 * of using the erasure, but this version is actually less restrictive
			 */
	...
	{
		return new DLDummyDescription<Name, Klass, Role>(DLTermOrder.DL_BEFORE_SOME_RESTRICTION);
	}

	public IDLRestriction<Name, Klass, Role> getAfter()
	{
		return new DLDummyDescription<Name, Klass, Role>(DLTermOrder.DL_AFTER_SOME_RESTRICTION);
	}
}
