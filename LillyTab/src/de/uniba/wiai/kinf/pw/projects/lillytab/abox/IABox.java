...
/**
 * An ABox is a set of instance nodes associated with descriptions.
 * An axiom also references a set of global axioms, the {@link TBox} of the
 * ABox.
 * </p><p>
 * ABoxes are the primary target of a tableaux reasoner. During operation,
 * a tableaux reasoner will modify the description sets of the nodes in
 * the ABox and potentially add additional nodes to the ABox.
 * </p>
 * </p><p>
 * Nodes inside an {@link IABox} need to have a natural order. Note,
 * that the order of nodes in an ABox is not allowed to change, while
 * a node is part of the ABox's node set. If it is required to change the
 * order of a node, it has to be removed and re-inserted into the ABox.
 * This ensures, that any module that depends on the natural node order
 * can register itself as a node list listener (see {@link #getNodeSetListeners()}/)
 * and be informed of any required updates.
 * </p>
	...
	extends SortedSet<IABoxNode<Name, Klass, Role>>, IImmutable<IABox<Name, Klass, Role>>, Cloneable
{
	...
	/**
	 * <p>
	 * Create a new, anonymous nodeand add it to the current ABox.
	 * </p><p>
	 * Note, that this does NOT unfold global descriptions onto the
	 * newly created node.
	 * </p><p>
	 * The {@see NodeID} of the new node is garantueed to
	 * be unique for the current ABox even across modifications
	 * (i.e. node merges). 
	 * </p>
	 * @seealso #mergeNodes(de.uniba.wiai.kinf.pw.projects.lillytab.abox.IABoxNode, de.uniba.wiai.kinf.pw.projects.lillytab.abox.IABoxNode) 
	 * @return The newly created ABoxNode.
	 */
	...
	/**
	 * <p>
	 * Retrieve or create a new named {@link IABoxNode} referencing the specified
	 *	{@literal individual}. If the individual did not exist, it will be created
	 *  and added to the current Abox.
	 * </p>
	 * The {@see NodeID} for newly created nodes is garantueed to
	 * be unique for the current ABox even across modifications
	 * (i.e. node merges). 
	 * </p>
	 * @seealso 
	...
	/**
	 * <p>
	 * The node map maps node names and node IDs to actual nodes.
	 * </p><p>
	 * As the current ABox may be cloned, references to ABox nodes
	 * should never be to the real class but indirectly via
	...
	/**
	 * <p>
	 * Join the two nodes {@literal node1} and {@literal node2} into
	 * a single node. The result of the join is a single node
	 * containing all concepts (and thus names) of both nodes.
	 * </p><p>
	 * The returned node is the target node of the join,
	 * which is <em>always</em> the smaller (as defined by
	 * the natural order) of both nodes.
	 * </p><p>
	 * After the join operation, only the target node of the returned
	 * {@link NodeMergeInfo} is a valid node inside the current ABox, all other nodes
	 * have been removed and should not be accessed any more.
	 * </p><p>
	 * The smaller node is returned, as we want to make sure,
	 * that we never process any later node before we are done
	 * processing earlier nodes. If we join to the smaller
	 * node, we always move in to the direction of smaller nodes,
	 * thus we may revisit nodes, but nether jump over nodes.
	 * </p>
	 *
	 * @param node1 First node to merge.
	 * @param node2 Second node to merge.
	 * @return The merged node, normally either {@literal node1} or {@literal node2}.
	 * @throws EInconsistentOntologyException The merge created an inconsistency.
	 */
	...
	/**
	 * <p>
	 * Retrieve the list of {@link ICollectionListener}s of the internal node set.
	 * </p><p>
	 * The {@link ICollectionListener} is informed, whenever a modification is made
	 * to the node list of the current ABox.
	 * </p><p>
	 * In particular, nodes are removed and re-added, when their order in the node list
	 * changes. Classes that depend on the order of nodes can register themselves
	 * as collection listeners.
	 * </p><p>
	...
	/**
	 * <p>
	 * Retrieve the list of {@link INodeMergeListener}s attached to this {@link IABox}.
	 * </p><p>
	 * A {@link INodeMergeListener} is informed, when two nodes are merged into one.
	 * </p><p>
	 * Note, that callers should take care when modifying the listener list,
	 * because the list also may also contain internal listeners. In particular,
	 * it is not safe to assume that the list is initially empty.
	 ...
	 * the abox's contents.
	 * </p><p>
	 * Contrary to {@link #hashCode() }, the abox's contents are taken into account
	 * and not only the object identity.
	 * </p>
	...
	/**
	 * <p>
	 * Compare two {@link IABox}es through deep inspection
	 * </p><p>
	...
	/**
	 * Get the list of all class references by any of the axioms
	 * in the current ABox, TBox or RBox.
	 * @return A list of classes from the ABox.
	 **/
	Set<Klass> getClassesInSignature();

	/**
	 * Get the list of all role references by any of the axioms
	 * in the current ABox, TBox or RBox.
	...
	/**
	 * Return the blocking state cache of the current ABox.
	 * The blocking state cache is used by reasoners to store
	 * cached information about the blocking status of nodes
	 * inside the ABox.
	 *
	...
	/**
	 * <p>
	 * Retrieve the list of unfold listeners.
	 * </p><p>
	 * {@link IUnfoldListener}s are informed, whenever a node's concept set is
	 * changed in result to an unfold (see {@link #addUnfoldedDescription(de.uniba.wiai.kinf.pw.projects.lillytab.terms.IDLClassExpression) }
	 * operation.
	 * </p><p>
	 * Note, that callers should take care when modifying the listener list,
	 * because the list also may also contain internal listeners. In particular,
