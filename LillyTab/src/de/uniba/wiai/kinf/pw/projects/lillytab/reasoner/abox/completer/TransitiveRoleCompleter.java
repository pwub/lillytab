	...
	implements ICompleter<Name, Klass, Role>
{
	...
	public ReasonerContinuationState completeNode(IABoxNode<Name, Klass, Role> node,
								IDecisionTree.Node<Branch<Name, Klass, Role>> branchNode) throws EReasonerException
	{
		...
		/**
		 * Remember, if we performed any modifications.
		 * If so, we should recheck the branch queue
		 * afterwards.
		 */
		...
		for (Role role : node.getPredecessors().keySet()) {
			/**
			 * Check for predecessors that are connected to the
			 * current node via a transitive role. 
			 **/
			if (rbox.hasRoleProperty(role, RoleProperty.TRANSITIVE) && node.getPredecessors().containsKey(role)) {
				/**
				 * If there is at least one predecessor connected to us via the current (transitive)
				 * role, walk the list of both predecessors for the current role and add
				 * links
				 *
				 */
				for (NodeID predID : node.getPredecessors().get(role)) {
					...
					if (succIDs != null) {
						for (NodeID succID : succIDs) {
							...
							/**
							 * If there is not yet a role link between successor and
							 * predecessor, add it.
							 **/
							if (!pred.getSuccessors().containsValue(role, succID)) {
								...
								wasModified = true;
								if (isTracing()) {
									logFinest("Added transitive %s-link from %s to %s", role, predID, succID);
								}
							}
						}
					}
