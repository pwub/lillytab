...
public class NodeConsistencyChecker<Name extends Comparable<? super Name>, Klass extends Comparable<? super Klass>, Role extends Comparable<? super Role>>
	...
	/**
	 * Determine if the union of both sets of descriptions contains a direct contradiction
	 * and return the first pair of contradicting terms.
	 *
		...
		throws EReasonerException, EInconsistentABoxException
	{
		...
		for (Role role : node.getSuccessors().keySet()) {
			if (rbox.hasRoleProperty(role, RoleProperty.FUNCTIONAL) && (node.getSuccessors().get(role).size() > 1))
				throw new ERoleCardinalityException(node, role,
					String.format("More than one successor for functional role '%s'", role));
		}
		for (Role role : node.getPredecessors().keySet()) {
			if (rbox.hasRoleProperty(role, RoleProperty.INVERSE_FUNCTIONAL) && (node.getPredecessors().get(role).size() > 1))
				throw new ERoleCardinalityException(node, role,
					String.format("More than one predecessor for inverse functional role '%s'", role));
		}
	}
		...
		throws EReasonerException, EInconsistentABoxException
	{
		if (node.isDatatypeNode()) {
			for (Map.Entry<Role, Collection<NodeID>> entry : node.getSuccessors().entrySet()) {
				...
				if (! node.isAnonymous()) {
					for (Name individual : node.getNames())
						if (!dtExpression.isValidValue(individual))
							throw new EInconsistentABoxNodeException(node, "'" + individual + " is not a valid value for datatype expression " + dtExpression);
					if (!dtExpression.isCompatibleValue(node.getNames()))
						throw new EInconsistentABoxNodeException(node, "Incompatible datatype values: " + node.getNames());
				}
			}
		}
	}
	...
	public void checkConsistency(IABoxNode<Name, Klass, Role> node) throws EReasonerException, EInconsistentABoxException
	{
		checkConsistency(node, null);
	}

	public void checkConsistency(IABoxNode<Name, Klass, Role> node,
		Collection<? extends IDLTerm<Name, Klass, Role>> extraDesc) throws EReasonerException, EInconsistentABoxException
	{
		final IABox<Name, Klass, Role> abox = node.getABox();
		final Collection<? extends IDLTerm<Name, Klass, Role>> inconsistent = getInconsistentTerms(abox, node.getTerms(), null);
	...
	{
		final Collection<? extends IDLTerm<Name, Klass, Role>> inconsistent = getInconsistentTerms(abox, descs, extraDescs);
	...
	{
		return isConsistent(abox, node.getTerms(), extraDesc) && hasProperFunctionalRoles(node) && isProperDatatypeNode(node);
	}
