...
public final class ConsistencyInfo<Name extends Comparable<? super Name>, Klass extends Comparable<? super Klass>, Role extends Comparable<? super Role>> {
	...
	public ConsistencyInfo(final ClashType clashType)
	{
		...
		_culprits = new HashSet<Set<Set<TermEntry<Name, Klass, Role>>>>();
	}
	...
	public void addCulprits(final IABoxNode<Name, Klass, Role> node, IDLTerm<Name, Klass, Role>... culprits)
	{
							...
							final Collection<? extends IDLTerm<Name, Klass, Role>> culprits)
	{
		...
		final Set<Set<TermEntry<Name, Klass, Role>>> culpritsSet = new Flat3Set<Set<TermEntry<Name, Klass, Role>>>();
		for (IDLTerm<Name, Klass, Role> culprit : culprits) {
			...
			culpritsSet.add(new FrozenFlat3Set<TermEntry<Name, Klass, Role>>(altSet));
		}
		_culprits.add(new FrozenFlat3Set<Set<TermEntry<Name, Klass, Role>>>(culpritsSet));
	}
