			...
			final IDecisionTree.Node<Branch<Name, Klass, Role>> branchNode) throws EReasonerException, EInconsistentABoxException
	{
		...
		while (iter.hasNext()) {
			...
			if (desc instanceof IDLUnion) {
				...
				final TermEntry<Name, Klass, Role> parentTerm = node.getABox().getTermEntryFactory().getEntry(node, desc);
				...
				/**
				 * If we needed to branch, but all branches are inconsistent.
				 * Don't throw Exception, when we did not find a branch point.
				 **/
				if (!branchActions.isEmpty()) {
					if (branchCreationInfos.isEmpty()) {
						/* TODO: correctly trace the terms responsible for inconsistencies and throw a more suitable exception */
						throw new EInconsistentABoxNodeException(node, "Cannot unfold union: " + desc);
					} else {
						if (isTracing())
