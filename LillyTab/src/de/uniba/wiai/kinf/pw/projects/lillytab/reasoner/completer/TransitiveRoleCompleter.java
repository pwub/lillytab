...
public class TransitiveRoleCompleter<Name extends Comparable<? super Name>, Klass extends Comparable<? super Klass>, Role extends Comparable<? super Role>>
												  ...
												  IDecisionTree.Node<Branch<Name, Klass, Role>> branchNode) throws EReasonerException
	{
		 ...
		 */
		for (Role incoming : node.getLinkMap().getIncomingRoles()) {
			/**
			 * Check for predecessors that are connected to the current node via a transitive role.
			 */
			if (rbox.hasRoleProperty(incoming, RoleProperty.TRANSITIVE) && node.getLinkMap().hasPredecessor(incoming)) {
				/**
				 ...
				 */
				for (NodeID predID : node.getLinkMap().getPredecessors(incoming)) {
					for (NodeID succID : node.getLinkMap().getSuccessors(incoming)) {
						 ...
						 */
						if (!pred.getLinkMap().hasSuccessor(incoming, succID)) {
							pred.getLinkMap().getAssertedSuccessors().put(incoming, succID);
							branch.touch(predID);
							branch.touch(succID);
							wasModified = true;
							if (isTracing()) {
