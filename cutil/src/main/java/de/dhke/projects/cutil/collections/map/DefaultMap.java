...
public class DefaultMap<K, V>
	...
	protected DefaultMap(final Map<K, V> backend, final Supplier<V> valueFactory)
	{
		_backend = backend;
		_valueFactory = (key) -> {
	...
	{
		_backend = backend;
		_valueFactory = (K key) -> defaultValue;
	}
	...
	public static <K, V> DefaultMap<K, V> decorate(final Map<K, V> backend, final Supplier<V> valueFactory)
	{
		return new DefaultMap<>(backend, valueFactory);
	}
