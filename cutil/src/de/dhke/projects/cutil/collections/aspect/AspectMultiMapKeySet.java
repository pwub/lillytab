	...
	implements Set<K>
{
		...
		implements Iterator<K>
	{
		...
		public void remove()
		{
			...
			Collection<V> values = new ArrayList<V>(_aspectMap.getDecoratee().get(_current));
			for (V value : values) {
				Map.Entry<K, V> entry = new DefaultMapEntry<K, V>(_current, value);
	...
	private boolean batchRemove(final Collection<?> c, final boolean retain)
	{
		...
		while (iter.hasNext()) {
			...
			if (c.contains(key) != retain) {
				...
				Collection<V> values = new ArrayList<V>(_aspectMap.get(key));
				iter.remove();
				wasRemoved = true;
				for (V value : values) {
					Map.Entry<K, V> entry = new DefaultMapEntry<K, V>(key, value);
